...
void TF1DMappingCanvas::setTF(float* keysData, int num)
{
    for(size_t i = 0; i < keys.size(); ++i)
        delete keys[i];
    keys.clear();
    for(int i = 0;i < num;i++){
        TF1DMappingKey* key = new TF1DMappingKey(keysData[i*5], QColor(keysData[i*5+1],keysData[i*5+2],keysData[i*5+3],keysData[i*5+4]));
...
void TF1DMappingCanvas::getTransferFunction(float* transferFunction,int* index, int* num, size_t dimension, float factor)
{
    int indexNum = 0;
    auto setColor = [&](float mr,float mg, float mb,  float ma, int mIndex){
        if(mIndex == index[indexNum - 1]) return;
        if(factor != 1)
          ma = 1.0 - pow(1.f - ma, factor);
        transferFunction[indexNum * 4 + 0] = mr;
        transferFunction[indexNum * 4 + 1] = mg;
        transferFunction[indexNum * 4 + 2] = mb;
        transferFunction[indexNum * 4 + 3] = ma;
        index[indexNum++] = mIndex;
    };

    auto setInterColor = [&](TF1DMappingKey* mLeftKey, TF1DMappingKey* mRightKey, int nIndex){
        float nr,ng,nb,na;
        float nValue = static_cast<float>(nIndex) / (dimension - 1);
        if(nValue < mLeftKey->getIntensity()){
            QColor nColor = mLeftKey->getColorL();
            nr = nColor.red() / 255.f;
            ng = nColor.green() / 255.f;
            nb = nColor.blue() / 255.f;
            na = nColor.alpha() / 255.f;
        }
        else if(nValue > mRightKey->getIntensity()){
            QColor nColor = mRightKey->getColorR();
            nr = nColor.red() / 255.f;
            ng = nColor.green() / 255.f;
            nb = nColor.blue() / 255.f;
            na = nColor.alpha() / 255.f;
            ...
            float mFraction  = (nValue - mLeftKey->getIntensity()) / (mRightKey->getIntensity() - mLeftKey->getIntensity());
            QColor leftDest  = mLeftKey->getColorR();
            QColor rightDest = mRightKey->getColorL();
            nr =leftDest.red()   / 255.0 + (rightDest.red()   - leftDest.red())   / 255.f * mFraction;
            ng = leftDest.green() / 255.0 + (rightDest.green() - leftDest.green()) / 255.f * mFraction;
            nb = leftDest.blue()  / 255.0 + (rightDest.blue()  - leftDest.blue())  / 255.f * mFraction;
            na = leftDest.alpha() / 255.0 + (rightDest.alpha() - leftDest.alpha()) / 255.f * mFraction;

        }
        setColor(nr,ng,nb,na,nIndex);
    };
    ...
    setColor(0,0,0,0,0);
    if(frontEnd != 0)
        setColor(0,0,0,0,frontEnd - 1);

    float r, g, b, a;
    std::vector<TF1DMappingKey*>::const_iterator keyIterator = keys.begin();
    bool firstKey = true;
    for(;keyIterator != keys.end();keyIterator++){
        TF1DMappingKey* leftKey;
        TF1DMappingKey* rightKey;
        QColor color;

        float intensity = (*keyIterator)->getIntensity();
        int tempIndex = static_cast<int>(std::floor(intensity * (dimension - 1) + 0.5));
        if(tempIndex < frontEnd) continue;
        if(tempIndex > backStart)
        {
            if(keyIterator==keys.begin())
            {
                setColor(0,0,0,0,dimension - 1);
                break;
            }
            leftKey  = *(keyIterator-1);
            rightKey = *keyIterator;
            setInterColor(leftKey,rightKey,backStart);
            setColor(0,0,0,0,backStart + 1);
            setColor(0,0,0,0,dimension - 1);
            break;
        }
        ...
        if(firstKey){
            ...
            if(keyIterator == keys.begin()){
                color = (*keyIterator)->getColorL();

                r = color.red() / 255.f;
                g = color.green() / 255.f;
                b = color.blue() / 255.f;
                a = color.alpha() / 255.f;
                setColor(r,g,b,a,frontEnd);
            }
            else{
                leftKey  = *(keyIterator-1);
                rightKey = *keyIterator;
                setInterColor(leftKey,rightKey,frontEnd);
            }
        }

        if(tempIndex == index[indexNum - 1]) continue;

        if(intensity * (dimension - 1) + 0.5 > tempIndex)
            color = (*keyIterator)->getColorR();
        else
            color = (*keyIterator)->getColorL();
        r = color.red() / 255.f;
        g = color.green() / 255.f;
